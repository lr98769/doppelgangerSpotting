...
Since NOR_12_H appears to be anomalous, we will be leaving out NOR_12_H from this experiment. 

```{r}
veri_results_dmd =  doppelgangerVerification(
  experimentPlanFilename = "cleaned_data/dmd_experiment_plan.csv",
  raw_data = doppel_hp_over$Batch_corrected,
  meta_data = dmd_meta,
  do.batch.corr = FALSE)
```
...
displayVerificationResults(functionalityResults = veri_results_dmd,
...
```{r}
temp = getPPCCDoppelgangers(dmd, dmd_meta, do.batch.corr = FALSE)
```
```{r}
...
```{r}
temp = doppel_hp_over$PPCC_df
temp = temp[temp$ClassPatient=="Different Class\n Different Patient",]
temp = temp[order(temp$PPCC, decreasing=TRUE),]
temp
```
```{r}
temp = doppel_hp_over$PPCC_df
temp = temp[temp$ClassPatient=="Same Class\n Different Patient" &
              temp$DoppelgangerLabel=="Doppelganger",]
temp = temp[order(temp$PPCC, decreasing=TRUE),]
temp
```
```{r}
veri_results_dmd_extra =  doppelgangerVerification(
  experimentPlanFilename = "cleaned_data/dmd_experiment_plan.csv",
  raw_data = dmd,
  meta_data = dmd_meta)
originalTrainValidNames =  c("Doppel_0", "Doppel_1","Doppel_2","Doppel_3", "Pos_Con_3", "Neg_Con")
...
displayVerificationResults(functionalityResults = veri_results_dmd_extra,
                originalTrainValidNames = originalTrainValidNames,
                newTrainValidNames = newTrainValidNames)
```
## Arrange plots
...
dmd_plot2 = displayVerificationResults(functionalityResults = veri_results_dmd,
                originalTrainValidNames = originalTrainValidNames,
                newTrainValidNames = newTrainValidNames)
